...
void updateClock(int h, int m, int s);
void clock_timer(int h, int m, int s);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : SEG0_Pin PB1 PB2 PB10
                           PB11 PB12 PB13 PB3
                           PB4 PB5 PB6 PB7
                           PB8 PB9 */
  GPIO_InitStruct.Pin = SEG0_Pin|GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_10
                          |GPIO_PIN_11|GPIO_PIN_12|GPIO_PIN_13|GPIO_PIN_3
                          |GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_6|GPIO_PIN_7
...
void updateClock(int h, int m, int s){
	static int prev_h = -1, prev_m = -1, prev_s = -1;
	h = h % 12;
	m = (m / 5) % 12;
	s = (s / 5) % 12;
	if (prev_h <= 0) display(h, OFF);
	if (prev_m <= 0) display(m, OFF);
	if (prev_s <= 0) display(s, OFF);
	display(h, ON);
	display(m, ON);
	display(s, ON);
	prev_h = h;
